...
static double           g_dIpsRef1[2] = { 0.0, 0.0 };
static double           g_dIpsRef2[2] = { 0, 420.0 };
static double           g_dIpsRef3[2] = { 300.0, 420.0 };
...
IPS_STS IpsTimer( void )
{
  ...
  ********************************************************/
  ...
  {
    btx = 1;
    ret = DWT_StartTx(&g_UWBHandle, DWT_START_RX_IMMEDIATE|DWT_RESPONSE_EXPECTED);
    if (ret == DWT_SUCCESS)
    {
      g_nTimeoutMs = 10;
      ...
      while (g_bSts == FALSE)
      {
        if (g_nTimeoutMs == 0)
        {
          ...
          if (0 == retry)
          {
            btx = 0;
            //TRACE ("IPS timeout\n");
            // Cycling through beacons when timeout
            if (g_nBc < 3)
              g_nBc++;
            else  
              g_nBc = 0;
  ...
  if (framelen <= RX_BUFFER_LEN)
  {
    ...
    if (g_RespMsg.b.Code == 0xE1 && g_RespMsg.b.DestAddr == g_TxMsg.b.DestAddr)
    {
      ...
      //rtd_init = DWT_ReadRxTimeStamplo32() - DWT_ReadTxTimeStamplo32();
      //rtd_resp = g_RespMsg.b.TxTimestamp - g_RespMsg.b.RxTimestamp;
      TRACE ("Poll Tx: %d, Poll Rx: %d, Resp Rx: %d, Rest Tx: %d\n", poll_tx_ts, poll_rx_ts, resp_rx_ts, resp_tx_ts);
      ...
      // Converting tof to distance
      dist = ((tof * SPEED_OF_LIGHT * 10000.0)/100) - 40;
      // Distance filtering
      ...
      if (dist <= 30000)
      {
        if (dist < 0)
          dist = 0;
        // g_dDistance is current distance
        // g_dDistance2 is previous distance
        // dist is next distance
        if (g_dDistance[g_nBc] == 0)
          g_dDistance[g_nBc] = dist;
        else if ((g_dDistance2[g_nBc] == 0) && (g_dDistance[g_nBc] != 0))
        {
          temp = g_dDistance[g_nBc];
          g_dDistance[g_nBc] = (g_dDistance[g_nBc] + dist) / 2;
          g_dDistance2[g_nBc] = temp;
        }
        else if ((g_dDistance[g_nBc] != 0) && (g_dDistance2[g_nBc] != 0))
        {
          if (dist > g_dDistance[g_nBc])
          {
            if ((dist - g_dDistance[g_nBc]) > 40)
              return res;
          }
          else if ((g_dDistance[g_nBc] - dist) > 40)
            return res;
          //if ((((g_dDistance[g_nBc]+ g_dDistance2[g_nBc] + dist)/3) - g_dDistance[g_nBc]) < 100)
          //{
          temp = g_dDistance[g_nBc];
          // If less than 60cm, unsure what kind of compensation to provide
          if (dist < 60)
          {
            g_dDistance[g_nBc] = (g_dDistance[g_nBc]+ g_dDistance2[g_nBc] + dist)/3;
          }
          //}
          // Else when distance more than 60cm, current assessment -40cm.
          else 
          {
            g_dDistance[g_nBc] = (g_dDistance[g_nBc]+ g_dDistance2[g_nBc] + dist)/3;
          }
    ...
    } // Calculation of distance end
  }
  // Cycling through 3 beacons
  if (bTxStatus)
  {
    if (g_nBc < 3)
      g_nBc++;
